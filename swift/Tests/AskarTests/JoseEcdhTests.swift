...
final class JoseEcdhTests: XCTestCase {
    ...
    func testEcdhEsDirect() throws {
        ...
        let encryptedMsg = try AskarEcdhEs(algId: enc, apu: apu, apv: apv).encryptDirect(
            encAlg: .a256Gcm, ephemeralKey: ephemKey, receiverKey: bobKey, message: message, nonce: nil, aad: protectedB64)

        let messageRecv = try AskarEcdhEs(algId: enc, apu: apu, apv: apv).decryptDirect(
            encAlg: .a256Gcm, ephemeralKey: ephemKey, receiverKey: bobKey, ciphertext: encryptedMsg.ciphertext(), tag: encryptedMsg.tag(), nonce: encryptedMsg.nonce(), aad: protectedB64)
    ...
    func testEcdhEsWrapped() throws {
        ...
        let encryptedMsg = try cek.aeadEncrypt(message: message, nonce: nil, aad: protectedB64)
        ...
        let messageRecv = try cekRecv.aeadDecrypt(
            ciphertext: encryptedMsg.ciphertext(), tag: encryptedMsg.tag(), nonce: encryptedMsg.nonce(), aad: protectedB64)
        XCTAssertEqual(message, Data(messageRecv))
    }
    ...
    func testEcdh1puDirect() throws {
        ...
        let encryptedMsg = try AskarEcdh1Pu(algId: enc, apu: apu, apv: apv).encryptDirect(
            encAlg: .a256Gcm, ephemeralKey: ephemKey, senderKey: aliceKey, receiverKey: bobKey, message: message, nonce: nil, aad: protectedB64)

        let messageRecv = try AskarEcdh1Pu(algId: enc, apu: apu, apv: apv).decryptDirect(
            encAlg: .a256Gcm, ephemeralKey: ephemKey, senderKey: aliceKey, receiverKey: bobKey, ciphertext: encryptedMsg.ciphertext(), tag: encryptedMsg.tag(), nonce: encryptedMsg.nonce(), aad: protectedB64)
        XCTAssertEqual(message, Data(messageRecv))
    }
    ...
    func testEcdh1puWrappedExpected() throws {
        ...
        let messageRecv = try cekRecv.aeadDecrypt(ciphertext: ciphertext, tag: ccTag, nonce: iv, aad: protectedB64)
