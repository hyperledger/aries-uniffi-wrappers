...
final class KeyTests: XCTestCase {
    ...
    func testAesCbcHmac() throws {
        ...
        let enc = try key.aeadEncrypt(message: data, nonce: nil, aad: Data("aad".utf8))
        let dec = try key.aeadDecrypt(ciphertext: enc.ciphertext(), tag: enc.tag(), nonce: enc.nonce(), aad: Data("aad".utf8))
    ...
    func testAesGcm() throws {
        ...
        XCTAssertEqual(params.tagLength, 16)
        let enc = try key.aeadEncrypt(message: data, nonce: nonce, aad: Data("aad".utf8))
        let dec = try key.aeadDecrypt(ciphertext: enc.ciphertext(), tag: enc.tag(), nonce: nonce, aad: Data("aad".utf8))
        XCTAssertEqual(data, dec)
    }

    func testBlsKeygen() throws {
        let key = try keyFactory.fromSeed(alg: .bls12381g1g2, seed: Data("testseed000000000000000000000001".utf8), method: .blsKeyGen)
    ...
    func testEd25519() throws {
        ...
        let sig = try key.signMessage(message: message, sigType: nil)
        XCTAssertTrue(try key.verifySignature(message: message, signature: sig, sigType: nil))
